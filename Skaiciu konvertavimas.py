    ...
    # Jei yra 16 arba 8 skaičiavimo sistema.
        ...
        # Jei skaičiavimos sistema yra šešioliktainė viską skirstisime į teatradas. Visa šios funkcijos dalis eina būtent šitaip:
            ...
            for ma in range(len(F) // 4):
                F1.append(
                    str(F[ma * 2 + mo]) + str(F[ma * 2 + 1 + mo]) + str(F[ma * 2 + 2 + mo]) + str(F[ma * 2 + 3 + mo]))
                mo += 2
...
dvejataineisesioliktaine = {"0000": "0", "0001": "1", "0010": "2", "0011": "3", "0100": "4", "0101": "5", "0110": "6",
...
# ______________________________________________________________________________________________________________________________________________________________________
# Antraštė.
Label(pg, text="Skaičiavimo sistemų konvertavimas", bg="#F6F6F6", font=("Times", "36", "bold underline")).pack(
    pady=plotis / 6)
# ______________________________________________________________________________________________________________________________________________________________________
...
wg = Spinbox(a, justify=CENTER, command=rasymas, from_=2, to=16, width=6, font="10", bg="#F6F6F6",
             textvariable=variable2, state="readonly")
wg.grid(row=0, column=4, padx=15)  # Antroji sistema
wgf = Spinbox(a, command=rasymas, justify=CENTER, from_=2, to=16, width=6, font="10", bg="#F6F6F6",
              textvariable=variable3, state="readonly")
...
e1 = Entry(a, state=DISABLED, justify=CENTER, width=int(45 * k), font="10", bg="#F6F6F6", textvariable=variable1)
...
# ______________________________________________________________________________________________________________________________________________________________________
'''
Šita programa labai įdomiai išėjo. Net nežinau kaip pasakyti. Ji yra labai "nepastovi" kodo atžvilgiu. Pvz.: vienur jau funkcijose yra perkonvertuojama iš bet kokios į bet kokią,
o kitur tas vyksta tik už funkcijos. Galbūt tas įvyko todėl, nes nežinojau nuo kokios programos dalies būtent reikėtų pradėti. Dar labai nusivylęs, kad neišėjo padaryti,
kad įvedus į e1, pasikeičia e. Žinoma būtų galima kopijuoti daug kodo ir jį taisyti, tačiau, mano manymu, nelabai apsimoka tą daryti, nes programa dvigubai paiilgės, o žmonių, kurie naudos
e1 įvesties laukelį bus labai mažai, nes tai vis dėl to yra skaičiavimo sistemos. Dar klaida, nes funkcijose rašiau tiesioginį kreipimasį į objektus pvz.:variable3.get(), tas sukėlė papildomų
sunkumų, kuriuos būtų buvę galima išvengti.
'''
